...
class AppPage(tk.Frame):
	...
	def clicked_item_inListBox(self, event):
		if not self.update_mode:
			...
			self.table_info[2][1].configure(text=hargabeli)
			self.table_info[3][1].configure(text=hargajual)
	...
	def create_detail_right_content(self):
		...
		for ProductID, info in self.current_data.items():
			info = [
				...
				['Stock :', info['stok']],
				['Harga Beli :', info['hargabeli']],
				['Harga Jual :', info['hargajual']]
			]
		self.table_info = []
	...
	def create_detail_right_footer(self):
		...
		features = ['Update', 'Delete', 'Add New', 'Profit Checker']
		commands = [self.clicked_update_btn, self.clicked_delete_btn, self.clicked_add_new_btn, self.clicked_profit_btn]
	...
	def clicked_update_btn(self):
		...
		self.table_info = []
		self.entry_update_data_vars = [tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar()]
		rows, columns = len(info), len(info[0]) # 3, 2
		for row in range(rows):
			aRow = []
			for column in range(columns):
	...
	def clicked_save_data_btn(self):
		...
		if confirm:
			...
			hargabeli = self.entry_update_data_vars[4].get()
			hargajual = self.entry_update_data_vars[5].get()
	...
	def clicked_save_add_new_data_btn(self):
		...
		if confirm:
			...
			hargajual = self.entry_update_data_vars[5].get()
			database = self.settings.database
			database = str(database)[1:-1]
			if productid in database:
				msg.showerror("Error", "ProductID already exists. Please use another ID.")
			else:
				new_data= {
					productid : {
						"itemname" : itemname,
	...
	def clicked_profit_btn(self):
		...
		for ProductID, info in self.current_data.items():
			info = [
				['Harga Beli :', info['hargabeli']],
				['Harga Jual :', info['hargajual']],
				['Profit :', int(info['hargajual']) - int(info['hargabeli'])]
			]
		self.table_info = []
		self.entry_update_data_vars = [tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar()]
		rows, columns = len(info), len(info[0]) # 3, 2
		...
		features = ['Okay']
		commands = [self.clicked_cancel_data_btn]
		self.buttons_features = []
		for feature in features:
			button = tk.Button(self.detail_update_footer, text=feature, bg="white", fg="black", bd=0, font=("Arial", 12, "bold"), command=commands[features.index(feature)])
			button.grid(row=0, column=features.index(feature), sticky="nsew", padx=20)
